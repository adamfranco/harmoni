 ...
 *
 * @version $Id: Qualifier.class.php,v 1.1 2008/04/24 20:51:43 adamfranco Exp $
 */
class AuthZ2_Qualifier 
	implements Qualifier 
{
	 ...
	 * @access public
	 */
	function __construct(Node $node, AuthZ2_AuthorizationCache $cache) {
		$this->_node =$node;
		$this->_cache =$cache;
	}
	...
	function getChildren () { 
		...
		while ($children->hasNext()) {
			...
			if (!isset($this->_cache->_qualifiers[$idValue])) {
				$qualifier = new AuthZ2_Qualifier($child, $this->_cache);
		...
		$obj = new AuthZ2_QualifierIterator($result);
	...
	function getParents () { 
		...
		while ($parents->hasNext()) {
			...
			if (!isset($this->_cache->_qualifiers[$idValue])) {
				$qualifier = new AuthZ2_Qualifier($parent, $this->_cache);
				$this->_cache->_qualifiers[$idValue] =$qualifier;
			}
