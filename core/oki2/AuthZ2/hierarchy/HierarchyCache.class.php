 ...
 *
 * @version $Id: HierarchyCache.class.php,v 1.1 2008/04/24 20:51:43 adamfranco Exp $
 **/

class AuthZ2_HierarchyCache {
	...
	function __construct($hierarchyId, $allowsMultipleParents, $dbIndex, $harmoni_db = null) {
		...
		$this->_nodeQuery->addColumn("keyword", "keyword", "az2_node_type");
		$this->_nodeQuery->addColumn("description", "type_description", "az2_node_type");
		$this->_nodeQuery->addTable("az2_node");
		$this->_nodeQuery->addOrderBy("az2_node.id");
		$joinc = "fk_type = "."az2_node_type.id";
		$this->_nodeQuery->addTable("az2_node_type", INNER_JOIN, $joinc);
		...
		if (!is_null($harmoni_db)) {
			...
			$query->addColumn("id", "id", "az2_node");
			$query->addColumn("display_name", "display_name", "az2_node");
			$query->addColumn("description", "description", "az2_node");
			...
			$this->getNodes_node_key = $query->addWhereEqual("az2_node.id", '');
			$this->getNodes_stmt = $query->prepare();
			
			$this->getNodesByType_domain_key = $byTypeQuery->addWhereEqual("az2_node_type.domain", '');
			$this->getNodesByType_authority_key = $byTypeQuery->addWhereEqual("az2_node_type.authority", '');
			$this->getNodesByType_keyword_key = $byTypeQuery->addWhereEqual("az2_node_type.keyword", '');
	...
	/**
	 * Set the hierarchy used by this cache
	 * 
	...
	/**
	 * Answer the hierarchy used by this cache
	 * 
	...
	public function getHierarchy () {
		if (!isset($this->hierarchy))
			throw new Exception("The Hierarchy for this cache must be set with setHierarchy()");
		
		return $this->hierarchy;
	}
	
	/**
	 * Set the AuthorizationManager used by this cache
	 * 
	...
	/**
	 * Answer the AuthorizationManager used by this cache
	 * 
	 * @return object AuthorizationManager
	 * @access public
	 * @since 4/22/08
	 */
	public function getAuthorizationManager () {
		if (!isset($this->authorizationManager))
			throw new Exception("The AuthorizationManager for this cache must be set with setAuthorizationManager()");
...
//		echo "</pre>\n";
	...
	public function getNodesFromDB ($nodeId) {
		...
		$this->_nodeQuery->addWhereEqual('az2_node.id', $nodeId);
	...
	public function getNodesFromDbByType (Type $type) {
		...
		$this->_nodeQuery->resetWhere();
		$this->_nodeQuery->addWhereEqual('domain', $type->getDomain());
		$this->_nodeQuery->addWhereEqual('authority', $type->getAuthority());
		$this->_nodeQuery->addWhereEqual('keyword', $type->getKeyword());
		$this->_nodeQuery->addWhereEqual("fk_hierarchy", $this->_hierarchyId);
	...
	function getAllNodes() {
		...
		$query->addColumn("keyword", "keyword", "az2_node_type");
		$query->addColumn("description", "type_description", "az2_node_type");
		$query->addColumn("fk_parent", "parent_id", "az2_j_node_node");

		$query->addTable("az2_node");
		$joinc = "fk_type = az2_node_type.id";
		$query->addTable("az2_node_type", INNER_JOIN, $joinc);
		$joinc = "az2_node.id = fk_child AND az2_node.fk_hierarchy = az2_j_node_node.fk_hierarchy";
		$query->addTable("az2_j_node_node", LEFT_JOIN, $joinc);

		$query->addWhereEqual("az2_node.fk_hierarchy", $this->_hierarchyId);
	...
	function getRootNodes() {
		...
		$joinc = "az2_node.id = fk_child and az2_node.fk_hierarchy = az2_j_node_node.fk_hierarchy";
		$query->addTable("az2_j_node_node", LEFT_JOIN, $joinc);
		$query->addColumn("fk_child", "join_id", "az2_j_node_node");
		$query->addWhereEqual("az2_node.fk_hierarchy", $this->_hierarchyId);
...
//			echo "</pre>\n";
			...
			while($queryResult->hasMoreRows()) {
				...
				if (!$this->_isCached($parentIdValue)) {
					...
					$parent = new AuthZ2_Node($parentId, $parentType, 
	...
	function getChildren(Node $node) {
		...
		if (!$this->_isCachedDown($idValue, 1)) {
			...
			// set the columns to select
			$query->addColumn("id", "id", "children");
			$query->addColumn("display_name", "display_name", "children");
			$query->addColumn("description", "description", "children");
			$query->addColumn("domain", "domain", "az2_node_type");
			$query->addColumn("authority", "authority", "az2_node_type");
			$query->addColumn("keyword", "keyword", "az2_node_type");
			$query->addColumn("description", "type_description", "az2_node_type");
			...
			// set the tables
			$query->addTable("az2_j_node_node", NO_JOIN, "", "parent");
			$joinc = "parent.fk_child = "."children.id";
			$query->addTable("az2_node", INNER_JOIN, $joinc, "children");
			$joinc = "children.fk_type = "."az2_node_type.id";
			$query->addTable("az2_node_type", INNER_JOIN, $joinc);
			
			$where = "parent.fk_hierarchy = '".$this->_hierarchyId."' AND parent.fk_parent = '".addslashes($idValue)."'";
			$query->addWhere($where);
			$query->addOrderBy("children.id");
			...
			if (count($nodesToExclude) > 0) {
				...
				$where = "children.id NOT IN ({$where})";
				$query->addWhere($where);
			}
...
//			echo "</pre>\n";
			...
			while($queryResult->hasMoreRows()) {
				...
				if (!$this->_isCached($childIdValue)) {
					...
					$childType = new HarmoniType($row['domain'], $row['authority'], 
												  $row['keyword'], $row['type_description']);
					$child = new AuthZ2_Node($childId, $childType, 
											  $row['display_name'], $row['description'], $this);
	...
	function isLeaf(Node $node) {
		...
		if ($this->_isCachedDown($idValue, 1)) {
			...
			// set the tables
			$query->addTable("az2_j_node_node");
			$query->addWhereEqual("az2_j_node_node.fk_hierarchy", $this->_hierarchyId);
...
//				throwError(new Error("Missing node object", "Hierarchy Cache"));
			...
			if (!isset($this->_infoCache[$nodeId->getIdString()])) {
				$this->_infoCache[$nodeId->getIdString()] 
					= new AuthZ2_TraversalInfo($nodeId,
												  $node->getDisplayName(),
												  $treeNodes[$key][1]);
			}
...
// 		echo "<br /><br /><br /><b>=== TraverseUpAncestory: Caching node # $idValue, $levels levels up</b><br />";
		...
		$query = new SelectQuery();
		$query->addColumn("*");
		$query->addTable("az2_node_ancestry");
		$query->addTable("az2_j_node_node", LEFT_JOIN, "az2_j_node_node.fk_hierarchy = az2_node_ancestry.fk_hierarchy AND fk_ancestor = fk_child");
// 		$query->setGroupBy(array("fk_ancestor"));
		$query->addOrderBy("level", DESCENDING);
		$query->addWhere("az2_node_ancestry.fk_hierarchy = '".addslashes($this->_hierarchyId)."' AND fk_node = '".addslashes($idValue)."'");
