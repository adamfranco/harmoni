 ...
 * @access public
 * @version $Id: HarmoniAuthorizationManager.class.php,v 1.19 2004/12/01 21:59:27 adamfranco Exp $
...
class HarmoniAuthorizationManager extends AuthorizationManager {
	...
	function &getRootQualifiers(& $qualifierHierarchyId) {
		...
		while ($nodes->hasNext()) {
			$node =& $nodes->next();
			$nodeId =& $node->getId();
			$qualifiers[] =& $this->getQualifier($nodeId);
		}
		
		return new HarmoniIterator($qualifiers);
	}
